...
class OrderFeedPage(BasePage):
    ...
    def click_feed_button(self):
        self.click_element_script(OrderFeedLocators.ORDER_FEED_BUTTON, 10)
        self.wait_for_element_visible(OrderFeedLocators.ALL_ORDER_COUNTER, 10)

    @allure.step('Получить номер первого заказа в ленте')
    def get_id_first_order_in_feed(self):
        return self.get_text(OrderFeedLocators.FIRST_ORDER_IN_FEED, 10)

    @allure.step('Клик на заказ')
    def click_order_in_feeds(self):
        self.click_element(OrderFeedLocators.FIRST_ORDER_IN_FEED, 10)

    @allure.step('Получить id заказа')
    def get_id_in_order_details(self):
        return self.get_text(OrderFeedLocators.ID_IN_ORDER_DETAILS, 10)

    @allure.step('Проверить появление окна с деталями заказа')
    def find_popup_order_detail(self):
        return self.find_element(OrderFeedLocators.ORDER_DETAIL, 10)

    @allure.step('Получить id заказа в окне с деталями заказа')
    def get_order_id(self):
        return self.get_text(MainFunctionalLocators.ID_IN_POP_UP_ORDER, 10)

    @allure.step('Закрыть окно с деталями заказа')
    def close_confirm_order_popup(self):
        self.click_element(MainFunctionalLocators.POP_UP_ORDER_CLOSE, 20)

    @allure.step('Ожидание появления номера заказа в окне заказа')
    def wait_order_number(self):
        for _ in range(10):
            if self.get_text(MainFunctionalLocators.ID_IN_POP_UP_ORDER, 10) == '9999':
                time.sleep(1)
            else:
                break
