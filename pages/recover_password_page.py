...
class RecoverPasswordPage(BasePage):
    ...
    def load_page(self):
        ...
        self.wait_for_element_visible(RecoverPasswordLocators.RECOVER_PASSWORD_BUTTON)

    @allure.step('Кликнуть на "Восстановить пароль"')
    def click_recover_password(self):
        self.click_element(RecoverPasswordLocators.RECOVER_PASSWORD_BUTTON)
        self.wait_for_element_visible(RecoverPasswordLocators.RECOVER_HEADER)
    ...
    def set_email(self):
        self.enter_text(RecoverPasswordLocators.EMAIL_FIELD_RECOVER, 'testmail@test.ma')

    @allure.step('Получение значения в поле "email"')
    def get_value_field(self):
        return self.get_value(RecoverPasswordLocators.EMAIL_FIELD_RECOVER, 'value')

    @allure.step('Клик по кнопке "Восстановить"')
    def click_recover_button(self):
        self.click_element(RecoverPasswordLocators.RECOVER_BUTTON)
        self.wait_for_element_visible(RecoverPasswordLocators.SAVE_BUTTON)

    @allure.step('Клик по кнопке "Скрыть/Показать"')
    def click_eye_button(self):
        self.click_element(RecoverPasswordLocators.EYE_BUTTON)

    @allure.step('Получение значения в поле "Пароль"')
    def get_class_password_field(self):
        return self.get_value(RecoverPasswordLocators.PASSWORD_FIELD, 'class')
